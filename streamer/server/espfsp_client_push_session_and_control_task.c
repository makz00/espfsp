...
static void handle_new_client_push_connection(espfsp_server_instance_t *instance, int sock)
{
    // Spawn ping task
    espfsp_comm_proto_run(&instance->client_push_comm_proto, sock);
}
...
void espfsp_server_client_push_session_and_control_task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            handle_new_client_push_connection(instance, sock);
